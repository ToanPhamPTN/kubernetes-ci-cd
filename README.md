...
`minikube start --insecure-registry="192.168.49.2:30400" --memory 8000 --cpus 2 --kubernetes-version=stable`
...
<!-- docker stop socat-registry; docker rm socat-registry; docker run -d --network host -e "REG_IP=$(minikube ip)" -e "REG_PORT=30400" --name socat-registry socat-registry -->
